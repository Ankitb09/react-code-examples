// All action types used

export const LOGIN = 'LOGIN';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_ERROR = 'LOGIN_ERROR';

export const USER_TYPE = 'USER_ACCESS_TYPE';
export const USER_TYPE_SUCCESS = 'USER_TYPE_SUCCESS';
export const USER_TYPE_ERROR = 'USER_TYPE_ERROR';

export const USER_SUBSCRIBE = 'USER_SUBSCRIBE';

export const FETCH_CATEGORIES = 'FETCH_CATEGORIES';
export const FETCH_CATEGORIES_SUCCESS = 'FETCH_CATEGORIES_SUCCESS';
export const FETCH_CATEGORIES_ERROR = 'FETCH_CATEGORIES_ERROR';

export const FETCH_BOOKLIST = 'FETCH_BOOKLIST';
export const FETCH_BOOKLIST_SUCCESS = 'FETCH_BOOKLIST_SUCCESS';
export const FETCH_BOOKLIST_ERROR = 'FETCH_BOOKLIST_ERROR';

export const FETCH_SELECTED_BOOK = 'FETCH_SELECTED_BOOK';
export const FETCH_SELECTED_BOOK_SUCCESS = 'FETCH_SELECTED_BOOK_SUCCESS';
export const FETCH_SELECTED_BOOK_ERROR = 'FETCH_SELECTED_BOOK_ERROR';

export const SELECT_CATEGORY = 'SELECT_CATEGORY';
